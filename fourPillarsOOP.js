...
class MainDish extends Food{
    ...
    get meatBased(){
    return (!this._meatBased) ? console.log(`${this.name} is a vegan dish`) : console.log(`${this.name} is not a vegan dish`)
    }
    howMuch(){
        if(this._meatBased) {
    ...
    const iceCream = new MainDish('Dessert','Ice-Cream', ['milk','sugar', 'corn-suryp'], 'American', false, 3)
    
    const burriito = new MainDish(('main', 'grande', ['cheese', 'beef', 'rice', 'beans'], 'handheld', true, 5.00)
    
    const pho = new MainDish('Main','Pho', ['noodles','mushroom', 'soup', 'salt', 'pepper', 'Siracha'], 'Vietnamese', false, 8)
    
    let dinner = [pho, iceCream, burriito]
