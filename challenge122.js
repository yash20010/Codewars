...
class Robot{
  constructor(){
    this.bank = ['thank' , 'you' , 'for' , 'teaching' , 'me', 'i' , 'already' , 'know', 'the', 'word' , 'do', 'not', 'understand', 'the', 'input']
  }
}

Robot.prototype.learnWord = function(word) {
  // Help Vicky learn some words!
  if(!word || word.toLowerCase().search(/[^a-z]/g) !== -1){
    return 'I do not understand the input'
  }
  if(this.bank.includes(word.toLowerCase())){
    return `I already know the word ${word}`
  }else{
    this.bank.push(word.toLowerCase())
